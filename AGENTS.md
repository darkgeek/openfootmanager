...
| `crates/engine/src/live_match/simulation.rs` | Live match engine loop and state machine |
| `crates/engine/src/live_match/snapshot.rs` | Match snapshots for UI state sync |
| `crates/engine/src/live_match/substitution.rs` | In-match substitution logic |
...
| `src/generator/definitions.rs` | Name definition structs and pool types |
| `src/generator/generation.rs` | `country_to_iso()`, `pick_name_from_def()`, `pick_nationality_from_def()` |
| `src/generator/world_io.rs` | Read/write generated world data to/from the database |
