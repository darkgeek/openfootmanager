...
/// SecondYellow is recorded as 0 yellow + 1 red in player_stats.
#[test]
fn second_yellow_gives_one_match_suspension() {
    let mut game = make_game_with_two_teams();
    // Second yellow = 0 yellow cards + 1 red card
    // (the second yellow already caused the red, so only count red)
    let report = report_with_cards(
        vec![("p1_def0", 0, 1)], // 0 yellow + 1 red (SecondYellow)
        vec![],
    );
    turn::apply_match_report(&mut game, 0, "team1", "team2", &report);
    ...
    assert_eq!(
        player.suspension_games_remaining, 1,
        "Player with SecondYellow (0 yellow + 1 red) should have 1 match suspension"
    );
}