...
pub fn ai_manage_team_condition(game: &mut Game) {
    ...
    for team in game.teams.iter_mut() {
        ...
        // Remember current focus as preferred when first needed (before switching to Recovery)
        if team.training_focus != TrainingFocus::Recovery 
            && team.preferred_training_focus == TrainingFocus::Recovery {
            // This only happens if preferred was never set (it's initialized to Physical,
            // but if game loaded and wasn't set, we set it here)
            if team.preferred_training_focus == TrainingFocus::Physical {
