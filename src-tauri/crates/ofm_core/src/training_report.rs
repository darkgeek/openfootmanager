    ...
    match existing_idx {
        Some(idx) => {
            ...
            log::info!("[training_report] Found existing snapshot from {} with {} players", 
                last_month, game.training_snapshots[idx].players.len());
            
            // Check if we already have a snapshot for this month
            if game.training_snapshots[idx].recorded_date.starts_with(&current_month) {
                log::info!("[training_report] Already have snapshot for this month, skipping");
                return;
            }

            // Calculate changes
            let changes = calculate_changes_for_team(&team_player_ids, &game.players, &game.training_snapshots[idx]);
